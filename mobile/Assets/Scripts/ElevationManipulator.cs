...
public class ElevationManipulator : Manipulator
{
    ...
    {
        m_Origin = transform.parent.localPosition;
        m_Origin.y = transform.parent.InverseTransformPoint(transform.parent.position).y;
        m_Origin = transform.parent.TransformPoint(m_Origin);
        OnStartElevationVisualization(m_Origin, transform.parent.position);
    }
    ...
    protected override void OnContinueManipulation(TwoFingerDragGesture gesture)
    {
        ...
        transform.parent.Translate(0.0f, elevationAmount, 0.0f);
        ...
        {
            transform.parent.position = transform.parent.TransformPoint(
                new Vector3(
        ...
        GetComponent<SelectionManipulator>().OnElevationChangedScaled(
            Mathf.Abs(transform.parent.position.y - m_Origin.y));
        OnContinueElevationVisualization(transform.parent.position);
    }
